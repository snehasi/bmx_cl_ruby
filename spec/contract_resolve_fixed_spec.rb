...
describe "payout closed issue" do
  describe "setup" do
    it "creates test elements" do
      ...
      `bmx issue sync FIXED --repo-uuid=#{repo_uuid}`
    ...
    it "set issue status to CLOSED (FIXED)" do
      issue_uuid = JSON.parse(`bmx issue list`)[0]["uuid"]
      result = `bmx issue sync FIXED --status=closed`
      expect($?.exitstatus).to eq(0)
      expect(result).to_not be_nil
    end
