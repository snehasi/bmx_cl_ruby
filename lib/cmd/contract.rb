...
class Contract < ThorBase
  ...
  desc "create CONTRACT_UUID", "create contract"
  ...
  def clone(contract_uuid)
    ...
    runput {contract.post_contract_contract_uuid_clone(contract_uuid, opts)}
  end
  ...
  def cancel(contract_uuid)
    contract = BmxApiRuby::ContractApi.new(client)
    runput {contract.post_contract_contract_uuid_cancel(contract_uuid)}
  end
